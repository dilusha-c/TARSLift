...
void AGV_STM32_DispatchCommand(const agv_packet_t *pkt) {
    switch (pkt->cmd) {
        ...
        case AGV_CMD_GET_STATUS: {
            uint16_t tx_len = agv_build_status(AGV_STATE_READY, 0x00, pkt->seq, tx_buffer, sizeof(tx_buffer));
            HAL_UART_Transmit(&huart2, tx_buffer, tx_len, 100);
            break;
        }
        ...
        default: {
            uint16_t tx_len = agv_build_ack(pkt->cmd, AGV_RESULT_OK, pkt->seq, tx_buffer, sizeof(tx_buffer));
            HAL_UART_Transmit(&huart2, tx_buffer, tx_len, 100);
            break;
        }
    }
...
static void MX_GPIO_Init(void)
{
  ...
  /*Configure GPIO pins : TOF1_XSHUT_Pin TOF2_XSHUT_Pin TOF3_XSHUT_Pin */
  GPIO_InitStruct.Pin = TOF1_XSHUT_Pin|TOF2_XSHUT_Pin|TOF3_XSHUT_Pin;
